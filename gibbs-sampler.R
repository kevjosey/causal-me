...
gibbs_dr <- function(s, star, y, s.id, id,
  ...
  gamma[1,] <- coef(glm(y ~ 0 + xa, family = poisson(), offset = offset))
  ...
  for(j in 2:(n.iter + n.adapt)) {
    ...
    for (k in 1:ncol(xa)){
      ...
      log.eps <- sum(dpois(y, exp(xa%*%gamma_ + offset), log = TRUE)) -
        sum(dpois(y, exp(xa%*%gamma[j,] + offset), log = TRUE)) +
  ...
  #   
  #   return(list(estimate = dr_out[1,], variance = dr_out[2,]))
  #   
  # }, mc.cores = 4)
  # 
  # est.mat <- do.call(rbind, lapply(out, function(o) o$estimate))
  # var.mat <- do.call(rbind, lapply(out, function(o) o$variance))
  # 
  # estimate <- colMeans(est.mat)
  # variance <- colMeans(var.mat) + (1 + 1/nrow(est.mat))*apply(est.mat, 2, var)
  # 
