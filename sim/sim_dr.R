...
simulate <- function(scenario, n.sim, a.vals){
  ...
  out <- mclapply(1:n.sim, function(i,...){
    ...
    bayes_hat <- try(bayes_erc(s = s, star = s_tilde, y = y, offset = offset, weights = weights,
                               s.id = s.id, id = id, w = w, x = x, family = family, a.vals = a.vals, 
                               h.gamma = h.gamma, h.a = h.a, span = span, scale = scale, shape = shape, rate = rate,
                               n.iter = n.iter, n.adapt = n.adapt, thin = thin), silent = TRUE)
    
    # estimates
    est <- rbind(predict_example(a = a.vals, x = x, out_scen = out_scen),
                 if (!inherits(bayes_hat, "try-error")) {bayes_hat$smooth_estimate} else {rep(NA, length(a.vals))},
                 if (!inherits(bayes_hat, "try-error")) {bayes_hat$dr_estimate} else {rep(NA, length(a.vals))})
    
    #standard error
    se <- rbind(if (!inherits(bayes_hat, "try-error")) {sqrt(bayes_hat$smooth_variance)} else {rep(NA, length(a.vals))},
                if (!inherits(bayes_hat, "try-error")) {sqrt(bayes_hat$dr_variance)} else {rep(NA, length(a.vals))})
    
    # coverage probability
    cp <- rbind(if (!inherits(bayes_hat, "try-error")) {as.numeric((est[2,] - 1.96*se[1,]) < est[1,] & (est[2,] + 1.96*se[1,]) > est[1,])} else {rep(NA, length(a.vals))},
                if (!inherits(bayes_hat, "try-error")) {as.numeric((est[3,] - 1.96*se[2,]) < est[1,] & (est[3,] + 1.96*se[2,]) > est[1,])} else {rep(NA, length(a.vals))})
    
    return(list(est = est, se = se, cp = cp))
    
  }, mc.cores = 30, mc.preschedule = TRUE)
  
  est <- abind(lapply(out, function(lst, ...) if (!inherits(lst, "try-error")) {lst$est} else {matrix(NA, ncol = length(a.vals), nrow = 3)}), along = 3)
  se <- abind(lapply(out, function(lst, ...) if (!inherits(lst, "try-error")) {lst$se} else {matrix(NA, ncol = length(a.vals), nrow = 2)}), along = 3)
  cp <- abind(lapply(out, function(lst, ...) if (!inherits(lst, "try-error")) {lst$cp} else {matrix(NA, ncol = length(a.vals), nrow = 2)}), along = 3)
  
  out_est <- t(apply(est, 1, rowMeans, na.rm = T))
  colnames(out_est) <- a.vals
  rownames(out_est) <- c("ERF","OM", "DR")
  ...
  filename <- paste0("~/Dropbox/Projects/ERC-EPE/Output/sim_dr/", paste(scenario, collapse = "_"),".RData")
