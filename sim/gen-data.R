...
gen_data <- function(n = c(400, 800), mult = c(5, 10), sig_agg = sqrt(2), sig_gps = 1, sig_pred = sqrt(0.5),
  ...
  if (out_scen == "b") {
    mu_out <- -3 - 0.5*u[,1] - 0.25*u[,2] + 0.25*u[,3] + 0.5*u[,4] +
      0.25*(a - 8) - 0.75*cos(pi*(a - 6)/4) - 0.25*(a - 10)*u[,1]
  } else { # y_scen == "b"
    mu_out <- -3 - 0.5*x[,1] - 0.25*x[,2] + 0.25*x[,3] + 0.5*x[,4] +
      0.25*(a - 8) - 0.75*cos(pi*(a - 6)/4) - 0.25*(a - 10)*x[,1]
  }
...
predict_example <- function(a.vals, x, out_scen = c("a", "b")) {
  ...
  for(i in 1:length(a.vals)) {
    ...
    if (out_scen == "b") {
      mu_out <- exp(-3 + u %*% c(-0.5,-0.25,0.25,0.5) + 0.25*(a.vec - 8) - 0.75*cos(pi*(a.vec - 6)/4) - 0.25*(a.vec - 10)*u[,1])
    } else { # out_scen == "a"
      mu_out <- exp(-3 + x %*% c(-0.5,-0.25,0.25,0.5) + 0.25*(a.vec - 8) - 0.75*cos(pi*(a.vec - 6)/4) - 0.25*(a.vec - 10)*x[,1])
    }
