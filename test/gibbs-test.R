...
for (i in 1:n.sim){
  ...
  blp_x <- erc(y = y, a = a_x, x = x, wts = wts, a.vals = a.vals, sl.lib = sl.lib, span = span)
  ...
  out_w <- mclapply.hack(1:length(a_list_x), function(k, ...){
    ...
    erc(y = y, a = a_list_w[[k]], x = x, wts = wts, a.vals = a.vals, sl.lib = sl.lib, span = span)
    
  }, mc.cores = 3)
  
  out_x <- mclapply.hack(1:length(a_list_x), function(k, ...){
    
    erc(y = y, a = a_list_x[[k]], x = x, wts = wts, a.vals = a.vals, sl.lib = sl.lib, span = span)
